...
router.post('/login', async (req: Request, res: Response) => {
  ...
  let pid = uuidv4();
  console.log(new Date().toLocaleString() + `: ${pid} - User ${email} requested for login`);

  if (!email || !EmailValidator.validate(email)) {
    console.log(new Date().toLocaleString() + `: ${pid} - User ${email} failed to login`);
  ...
  console.log(new Date().toLocaleString() + `: ${pid} - User ${email} successfully login`);
